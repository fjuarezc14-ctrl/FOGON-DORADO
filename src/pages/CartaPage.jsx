...
export default function CartaPage() {
  ...
  const abrirModal = (p = null) => {
    setEditProd(p
      ...
      : { id: '', nombre: '', categoria: 'Pollos', precio: '', tipoStock: 'ilimitado', stock: '' }
    );
    setModalOpen(true);
  };
    ...
    <section className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500">Administra los platos, precios y controla el stock. Guardado en Base de Datos.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {productosFiltrados.length === 0
          ...
          : productosFiltrados.map(p => {
              ...
              if (p.categoria === 'Guarniciones') { colorIco = 'text-emerald-500'; bgIco = 'bg-emerald-100'; }
              ...
              return (
                <div key={p.id} className={`bg-white rounded-3xl border border-slate-100 shadow-sm p-4 relative overflow-hidden transition-all hover:shadow-md ${isAgotado ? 'opacity-75 grayscale-[50%]' : ''}`}>
                  ...
                  <div className="flex justify-between items-start mb-3 relative z-0">
                    <div className={`w-12 h-12 rounded-2xl ${bgIco} flex items-center justify-center ${colorIco}`}><Icon className="w-6 h-6" /></div>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                {editProd.tipoStock === 'limitado' && (
                  <div>
                    ...
                    <input type="number" value={editProd.stock} onChange={e => setEditProd({ ...editProd, stock: e.target.value })} placeholder="Ej. 24" className="w-full border border-slate-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-amber-500 font-mono" />
                  </div>
                )}
              </div>
            </div>
