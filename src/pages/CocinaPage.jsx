...
export default function CocinaPage() {
  ...
  const despacharPedido = (indexMesa) => {
    let nuevasMesas = [...mesas];
    if(nuevasMesas[indexMesa]) {
      nuevasMesas[indexMesa].estado = 'Servido';
      if(nuevasMesas[indexMesa].pedidoData && nuevasMesas[indexMesa].pedidoData.items) {
        nuevasMesas[indexMesa].pedidoData.adicional = false;
        nuevasMesas[indexMesa].pedidoData.items.forEach(i => i.historial = true);
      }
      ...
      <section className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
        <div className="mb-6 md:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-xs md:text-sm text-slate-400">Tickets sincronizados directamente desde el Salón.</p>
          </div>
          ...
          {mesasCocina.length === 0 ? (
            ...
            return (
              <div key={idx} className="bg-white rounded-t-xl rounded-b shadow-2xl flex flex-col overflow-hidden transform transition-all hover:-translate-y-1 relative" style={{ minHeight: '300px' }}>
                ...
                <div className="p-4 bg-slate-50 mt-2 shrink-0 pb-6 relative z-10 border-t-2 border-dashed border-slate-300">
                  <button onClick={() => despacharPedido(m.num - 1)} className="w-full py-3.5 bg-emerald-500 hover:bg-emerald-600 text-slate-900 font-black uppercase tracking-widest rounded-xl transition-all shadow-md flex items-center justify-center gap-2 text-sm">
